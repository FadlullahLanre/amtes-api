...
const login = catchAsync(async (req, res, next) => {
    ...
    if (!matric_number || !password) {
        return next(new AppError('Please provide matric number and password', 400));
    }

    const user = await User.findOne({ matric_number }).select('+password +active');
...
const updateMe = catchAsync(async (req, res, next) => {
    ...
    const filteredBody = filterObj(req.body, 'phoneNumber', 'twitter', 'facebook', 'linkedin', 'bio', 'occupation', 'location');
